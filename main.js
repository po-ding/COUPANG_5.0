/**
 * main.js - CargoNote Final Complete Version (Salary Added)
 * 
 * [통합 수정 사항]
 * 1. 일별/주별 조회 시 상단 월간 요약 박스 제거
 * 2. 리스트 디자인: 상하차 수직 배치, 메모 표시, 3단 분류
 * 3. SMS 등록 시 시간 미입력 -> '운행 등록' 상태로 저장
 * 4. 수정/등록 모드에 따른 입력창 표시/숨김 자동화
 * 5. 지역 관리 및 삭제 팝업 커스텀 처리
 * 6. 내역 출력 시 운임 입력 필드 추가
 * 7. (New) 급여 관리 기능 추가 (데이터 관리 섹션)
 * 8. (New) 내역 출력 및 수입 합계에 급여 반영
 * 9. (New) '04시 기준' 텍스트 일괄 제거
 */

// ==========================================
// 0. GLOBAL FUNCTIONS (HTML onclick 연결용)
// ==========================================
...
window.openSettings = function() {
    ...
    try {
        ...
        if (typeof displaySalaryManager === 'function') displaySalaryManager(); // 급여 관리 UI 표시
        ...
        console.error("설정 화면 데이터 갱신 중 오류:", e);
    }
};
...
window.closeSettings = function() {
    ...
    if (editId && editId.value && editIndicator) {
...
window.editCenter = function(oldName) {
    ...
    const newAddr = prompt("주소 수정:", loc.address || "");
    if (newAddr === null) return; 

    const newMemo = prompt("메모 수정:", loc.memo || "");
...
window.addNewCenterFromInput = function() {
    let input = document.getElementById('add-center-input');
    if (!input) input = document.getElementById('new-center-name');
...
// ==========================================
// SMS & SALARY DISPLAY FUNCTIONS
// ==========================================

function displaySavedSmsNumber() {
    const savedNum = localStorage.getItem('target_sms_number');
    const displayDiv = document.getElementById('saved-sms-number-display');
    
    if (!displayDiv) {
        const parent = document.getElementById('data-management-body');
        if(parent) {
            const newDiv = document.createElement('div');
            newDiv.id = 'saved-sms-number-display';
            ...
            renderSmsNumberContent(newDiv, savedNum);
        }
    } else {
        renderSmsNumberContent(displayDiv, savedNum);
    }
}
...
function renderSmsNumberContent(element, num) {
    if (num) {
        element.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center; background:#f1f8ff; padding:8px; border-radius:4px;">
                <span>저장된 번호: <strong>${num}</strong></span>
                <button type="button" onclick="window.deleteSavedSmsNumber()" style="background:#dc3545; color:white; border:none; border-radius:3px; padding:4px 10px; font-size:0.8em; cursor:pointer;">삭제</button>
            </div>`;
    } else {
        element.innerHTML = "<div style='padding:5px; color:#666;'>저장된 발신번호가 없습니다.</div>";
    }
}

window.deleteSavedSmsNumber = function() {
    if(confirm('저장된 발신번호를 삭제하시겠습니까?')) {
        localStorage.removeItem('target_sms_number');
        displaySavedSmsNumber();
        const inputEl = document.getElementById('sms-target-number');
...
// ==========================================
// 급여 관리 (SALARY MANAGEMENT) - 수정됨
// ==========================================

// ==========================================
// [앱 빌드용 최종] 급여 관리 기능 (SALARY MANAGEMENT)
// ==========================================
...
function displaySalaryManager() {
    ...
    if (!container) {
        ...
        container.id = 'salary-management-container';
        // 모바일 터치 영역 확보를 위한 스타일 조정
    ...
    // 연도 옵션 생성
    let yearOpts = '';
    for(let i=0; i<3; i++) yearOpts += `<option value="${currentYear-i}">${currentYear-i}년</option>`;
    
    // 월 옵션 생성
    let monthOpts = '';
    for(let i=1; i<=12; i++) monthOpts += `<option value="${String(i).padStart(2,'0')}">${i}월</option>`;

    // 리스트 아이템 생성
    let listHtml = '';
    const sortedKeys = Object.keys(MEM_SALARIES).sort().reverse();
    
    sortedKeys.forEach(key => {
        const amount = MEM_SALARIES[key];
        // 앱 가독성을 위한 리스트 디자인 (터치하기 좋게 패딩 증가)
        listHtml += `
            <div style="display:flex; justify-content:space-between; align-items:center; background:#f9f9f9; padding:12px; border-radius:6px; margin-bottom:8px; border:1px solid #eee;">
                <div style="font-size:1em; color:#333;">
                    <strong style="margin-right:5px;">${key}</strong>
                    <div style="color:#007bff; font-weight:bold; font-size:1.1em; margin-top:2px;">
                        ${formatToManwon(amount)} 만원
                    </div>
                </div>
                <div style="display:flex; gap:8px;">
                    <button type="button" onclick="window.editSalary('${key}', ${amount})" 
                        style="background:#17a2b8; color:white; border:none; border-radius:4px; padding:8px 12px; font-weight:bold; font-size:0.9em;">수정</button>
                    <button type="button" onclick="window.deleteSalary('${key}')" 
                        style="background:#dc3545; color:white; border:none; border-radius:4px; padding:8px 12px; font-weight:bold; font-size:0.9em;">삭제</button>
                </div>
            </div>
        `;
    });
    ...
    if(!listHtml) {
        listHtml = `<div style="color:#999; text-align:center; padding:20px; background:#f8f8f8; border-radius:5px;">
                        내역이 없습니다.<br>급여를 등록해주세요.
                    </div>`;
    }

    // HTML 렌더링 (앱 키패드 호환성을 위해 type="tel" inputmode="numeric" 사용)
    container.innerHTML = `
        <h4 style="margin-bottom:12px; font-weight:bold; color:#333; font-size:1.1em;">💰 급여 설정 (월 고정수입)</h4>
        
        <div style="display:flex; gap:5px; margin-bottom:8px;">
            <select id="salary-set-year" style="flex:1; padding:12px; border:1px solid #ccc; border-radius:5px; font-size:1em; background:#fff;">${yearOpts}</select>
            <select id="salary-set-month" style="flex:1; padding:12px; border:1px solid #ccc; border-radius:5px; font-size:1em; background:#fff;">${monthOpts}</select>
        </div>
        
        <div style="display:flex; gap:5px; margin-bottom:20px;">
            <input type="tel" id="salary-set-amount" 
                   inputmode="numeric" 
                   placeholder="금액 입력 (원)" 
                   oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                   style="flex:2; padding:12px; border:1px solid #007bff; border-radius:5px; font-size:1.1em; font-weight:bold;">
            <button type="button" onclick="window.saveSalary()" 
                    style="flex:1; padding:12px; background:#007bff; color:white; border:none; border-radius:5px; cursor:pointer; font-weight:bold; font-size:1em;">
                    저장
            </button>
        </div>
        
        <div style="max-height:300px; overflow-y:auto; border-top:2px solid #f1f1f1; padding-top:15px; -webkit-overflow-scrolling: touch;">
            ${listHtml}
        </div>
    `;
...
window.saveSalary = function() {
    ...
    const key = `${y}-${m}`;
    MEM_SALARIES[key] = amount;
    
    saveData(); // 데이터 저장
    displaySalaryManager(); // 리스트 새로고침
    
    // [핵심] 저장 즉시 메인 화면의 통계 수치 업데이트
...
window.deleteSalary = function(key) {
    // 앱에서는 confirm 창이 투박할 수 있으나 기능상 가장 확실함
...
function updateLocationData(name, address, memo) {
    ...
    if (address || memo) {
        MEM_LOCATIONS[trimmed] = { ...(MEM_LOCATIONS[trimmed] || {}), address: address || '', memo: memo || '' };
    }
    saveData();
}
    ...
    try { 
        ...
        if (navigator.clipboard) {
            navigator.clipboard.writeText(text)
                .then(() => showToast(msg || '주소가 복사되었습니다.'))
...
function displayCenterList(filter='') {
    ...
    list.forEach(c => {
        ...
        div.innerHTML=`<div class="info"><span class="center-name">${infoText}</span>
            <div class="action-buttons">
                <button type="button" class="center-action-btn btn-edit" style="background:#17a2b8;" onclick="window.editCenter('${c}')">수정</button>
                <button type="button" class="center-action-btn btn-delete" style="background:#dc3545;" onclick="window.deleteCenter('${c}')">삭제</button>
            </div>
        </div>`;
...
function updateAddressDisplay() {
    ...
    const buildInfo = (label, val, color) => {
        ...
        let h = `<div style="margin-bottom:5px; padding-bottom:5px; border-bottom:1px dashed #ccc;">
            <span style="font-weight:bold; color:${color};">[${label}] ${val}</span>`;
    ...
    if (html !== '') displayEl.style.display = 'block';
...
function resetForm() {
    ...
    // 등록 모드: 섹션 숨김
    const dtField = document.getElementById('datetime-info-fieldset');
    if(dtField) dtField.style.display = 'none';

    const typeLegend = document.getElementById('legend-type');
    if(typeLegend) {
        const typeField = typeLegend.closest('fieldset');
        if(typeField) typeField.style.display = 'none';
    }

    const sectionsToHide = ['transport-details', 'cost-info-fieldset'];
    ...
    const btnIds = ['toggle-basic-info-btn', 'toggle-location-section-btn', 'toggle-cost-section-btn'];
    btnIds.forEach(id => {
...
function editRecord(id) {
    ...
    // 수정 모드: 섹션 강제 표시
    const dtField = document.getElementById('datetime-info-fieldset');
    const dtBody = document.getElementById('body-datetime');
    if(dtField) {
        dtField.style.display = 'block';
        dtField.classList.remove('hidden');
    }
    if(dtBody) dtBody.style.display = 'block'; 

    const typeLegend = document.getElementById('legend-type');
    const typeBody = document.getElementById('body-type');
    if(typeLegend) {
        const typeField = typeLegend.closest('fieldset');
        if(typeField) {
    ...
    if(typeBody) typeBody.style.display = 'block'; 

    const sectionsToShow = ['transport-details', 'cost-info-fieldset'];
    sectionsToShow.forEach(sid => {
    ...
    validLines.forEach((line, lineIdx) => {
        ...
        const buildLocInput = (label, id, value, color) => {
            ...
            return `<div style="flex:1; display:flex; flex-direction:column; gap:4px;">
                    ...
                    <input type="text" id="${id}-addr" value="${locInfo?.address||''}" placeholder="주소" style="border:1px solid #ddd; padding:4px; font-size:0.8em; background:${locInfo?.address?'#f9f9f9':'#fff0f0'};">
                    <input type="text" id="${id}-memo" value="${locInfo?.memo||''}" placeholder="메모" style="border:1px solid #ddd; padding:4px; font-size:0.8em;">
                </div>`;
        };

        itemDiv.innerHTML = `<div style="display:flex; gap:10px; margin-bottom:5px;">${buildLocInput('상차지', `from-${lineIdx}`, finalFrom, '#007bff')}<div style="align-self:center; font-weight:bold; color:#ccc;">▶</div>${buildLocInput('하차지', `to-${lineIdx}`, finalTo, '#dc3545')}</div>
            <button type="button" class="sms-save-btn" 
            style="width:100%; padding:5px 0; margin-top:5px; background:#28a745; color:white; border:none; border-radius:3px; cursor:pointer; font-size:0.85em; font-weight:bold; text-align:center;"
            onclick="window.registerParsedTripWithInfo(this, ${lineIdx})">저장</button>`;
...
function handleSmsNameInput(input, addrId, memoId) {
    ...
    if (loc && addrInput && memoInput) {
        ...
        addrInput.style.backgroundColor = '#f9f9f9'; 
    } else if (addrInput) {
        addrInput.value = '';
...
function registerParsedTripWithInfo(btn, lineIdx) {
    ...
    addRecord({
        ...
        distance: MEM_DISTANCES[key] || 0, 
        income: MEM_FARES[key] || 0,
...
async function performOCR(file) {
    ...
    statusDiv.textContent = "영수증 분석 중... (5~10초 소요)";
    ...
    try {
        ...
        resultContainer.innerHTML = `
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:5px; margin-bottom:10px;">
                <input type="date" id="ocr-date" style="padding:8px; border:1px solid #ddd; border-radius:4px;">
                <input type="time" id="ocr-time" style="padding:8px; border:1px solid #ddd; border-radius:4px;">
                <input type="number" id="ocr-cost" placeholder="주유금액" style="padding:8px; border:1px solid #ddd; border-radius:4px;">
                <input type="number" id="ocr-subsidy" placeholder="보조금액" style="padding:8px; border:1px solid #ddd; border-radius:4px;">
                <input type="number" id="ocr-liters" placeholder="주유리터" step="0.01" style="padding:8px; border:1px solid #ddd; border-radius:4px;">
                <input type="number" id="ocr-price" placeholder="주유단가" style="padding:8px; border:1px solid #ddd; border-radius:4px;">
                <input type="number" id="ocr-remaining" placeholder="잔여한도" step="0.01" style="padding:8px; border:1px solid #ddd; border-radius:4px;">
                <input type="number" id="ocr-net-cost" placeholder="실지출(자동)" readonly style="padding:8px; border:1px solid #ddd; border-radius:4px; background:#eee;">
            </div>
            <div style="display:flex; gap:5px;">
                <button onclick="document.getElementById('ocr-result-container').classList.add('hidden')" style="flex:1; padding:10px; background:#6c757d; color:white; border:none; border-radius:4px;">초기화</button>
                <button id="btn-save-ocr-final" style="flex:1; padding:10px; background:#28a745; color:white; border:none; border-radius:4px; font-weight:bold;">저장하기</button>
            </div>
        `;
        ...
        console.error(error);
        statusDiv.textContent = "이미지 분석 실패. 다시 시도해주세요.";
    }
}
...
function saveOcrData() {
    ...
    addRecord({
        ...
        time: document.getElementById('ocr-time').value,
        type: '주유소',
        cost: cost,
        liters: safeFloat(document.getElementById('ocr-liters').value),
        unitPrice: safeInt(document.getElementById('ocr-price').value),
        brand: '기타',
        subsidy: safeInt(document.getElementById('ocr-subsidy').value) 
    });
...
function createSummaryHTML(title, records) {
    ...
    records.forEach(r => {
        ...
        if (r.type === '공차이동') {
            totalEmptyDist += safeFloat(r.distance);
        }
    });
    ...
    items.forEach(item => {
        html += `<div class="summary-item" style="border:1px solid #ddd; padding:5px; text-align:center; border-radius:5px;">
                    <span class="summary-label" style="display:block; font-size:0.8em; color:#666;">${item.label}</span>
                    <span class="summary-value ${item.className || ''}" style="font-weight:bold; font-size:0.9em;">${item.value}</span>
                 </div>`;
    });
    html += `</div>`;
    return html;
...
function displayTodayRecords(date) {
    ...
    const oldTbody = todayTable.querySelector('tbody');
    if(oldTbody) oldTbody.remove();
    
    const thead = todayTable.querySelector('thead');
    ...
    displayList.forEach(r => {
        if (['화물운송','대기','운행취소','공차이동'].includes(r.type)) {
            ...
            if(isCompleted) completedList.push(r);
            else ongoingList.push(r);
        } else {
            otherList.push(r);
        }
    });
    ...
    const createRow = (r, isHidden = false) => {
        ...
        if (r.time) { 
            ...
            if(r.type === '운행취소') endTime = '취소';
            else if(!endTime) endTime = '진행중';
        } else {
            endTime = '-';
        }
        ...
        if(r.type === '공차이동') {
            ...
            distInfo = `<span style="color:#856404; font-weight:bold;">[공차]</span> ` + distInfo;
        }

        let html = `
        <td colspan="7" style="padding:15px 10px; border-bottom:1px solid #ddd;">
            <div style="margin-bottom: 8px;">
                <div class="location-clickable" data-center="${r.from||''}" style="font-size:1.3em; font-weight:bold; color:#007bff; cursor:pointer; line-height:1.2;">
                    ${r.from||'-'}
                </div>
                ${fromMemoHtml}
            </div>
            <div style="font-size:1.0em; color:#ccc; margin-bottom: 8px; line-height:1;">
                ➜ 
            </div>
            <div style="margin-bottom: 12px;">
                <div class="location-clickable" data-center="${r.to||''}" style="font-size:1.3em; font-weight:bold; color:#28a745; cursor:pointer; line-height:1.2;">
                    ${r.to||'-'}
                </div>
                ${toMemoHtml}
            </div>
            <div style="border-top:1px solid #eee; padding-top:8px; display:flex; justify-content:space-between; align-items:flex-end; font-size:0.95em; color:#444;">
                <div>
                    <span style="font-weight:bold;">${timeDisplay}</span> 
                    <span style="color:#888;">~ ${endTime}</span> 
                    ${duration ? `<span style="font-size:0.85em; color:#666;">(${duration})</span>` : ''}
                </div>
                <div style="text-align:right;">
                    <span style="margin-right:8px; font-weight:bold; color:#555;">${distInfo}</span>
                    ${money}
                </div>
            </div>
        </td>`;
    ...
    if (registeredList.length > 0) {
        ...
        headerTr.innerHTML = `<td colspan="7" style="background:#e3f2fd; font-weight:bold; padding:8px; color:#0d47a1;">📋 운행 등록 (대기중)</td>`;
        tbody.appendChild(headerTr);
        registeredList.forEach(r => tbody.appendChild(createRow(r)));
    }

    if (ongoingList.length > 0) {
        const headerTr = document.createElement('tr');
        headerTr.innerHTML = `<td colspan="7" style="background:#e8f5e9; font-weight:bold; padding:8px; color:#1b5e20;">🚚 진행 중</td>`;
        tbody.appendChild(headerTr);
        ongoingList.forEach(r => tbody.appendChild(createRow(r)));
    }
    ...
    otherList.forEach(r => {
        ...
        tr.innerHTML = `<td colspan="7" style="padding:10px; border-bottom:1px solid #ddd; color:#555;">
            [${r.type}] ${r.expenseItem || r.supplyItem || r.brand || ''} <span style="float:right; font-weight:bold;">${money}</span>
            <div style="font-size:0.85em; color:#888;">${r.time}</div>
        </td>`;
    ...
    if(completedList.length > 0) {
        ...
        toggleTr.innerHTML = `
            <td colspan="7" style="padding:0; border:none;">
                <button type="button" 
                    onclick="document.querySelectorAll('.completed-row').forEach(el => el.classList.toggle('hidden')); this.textContent = this.textContent.includes('보기') ? '▲ 완료된 운행 접기' : '▼ 완료된 운행 보기 (${completedList.length})';"
                    style="width:100%; padding:12px; background:#f8f9fa; border:none; border-bottom:1px solid #ddd; color:#495057; font-weight:bold; cursor:pointer; text-align:left; font-size:1em;">
                    ▼ 완료된 운행 보기 (${completedList.length})
                </button>
            </td>`;
        tbody.appendChild(toggleTr);
        
        completedList.forEach(r => tbody.appendChild(createRow(r, true)));
    }
...
function displayDailyRecords() {
    ...
    // 일별 조회에서는 월간 요약 제거
    if(dailySummaryDiv) dailySummaryDiv.innerHTML = "";
...
function displayWeeklyRecords() {
    ...
    // 주별 조회에서는 월간 요약 제거
    if(weeklySummaryDiv) weeklySummaryDiv.innerHTML = "";
...
function displayMonthlyRecords() {
    ...
    Object.keys(MEM_SALARIES).forEach(k => {
        if(k.startsWith(year)) totalYearSalary += (MEM_SALARIES[k] || 0);
    });
    
    // createSummaryHTML로는 급여를 반영하기 어려우므로 수동으로 HTML 생성하거나
    // 가상의 레코드를 만들어서 넘겨야 함. 여기선 텍스트만 추가하는 방식 사용
    let summaryHtml = createSummaryHTML(`${year}년`, yearRecords);
    if(totalYearSalary > 0) {
        summaryHtml += `<div style="margin-top:5px; text-align:right; color:#007bff; font-weight:bold; font-size:0.9em;">➕ 연간 급여 합계: ${formatToManwon(totalYearSalary)} 만원 포함</div>`;
    }
    ...
    Object.keys(months).sort().reverse().forEach(m => {
        ...
        // 월별 급여 추가
        const monthlySalary = MEM_SALARIES[m] || 0;
        inc += monthlySalary;
...
function displayCurrentMonthData() {
    ...
    // UTC 변환 오류 수정: 로컬 시간 기준 연-월 문자열 생성
    ...
    // 현재 월 급여 반영
    const monthlySalary = MEM_SALARIES[currentPeriod] || 0;
    inc += monthlySalary;

    const setTxt = (id, txt) => { const el = document.getElementById(id); if(el) el.textContent = txt; };
    setTxt('current-month-title', `${parseInt(currentPeriod.split('-')[1])}월 실시간 요약`);
...
function renderMileageSummary(period = 'monthly') {
    ...
    if (period === 'monthly') {
        for (let i = 11; i >= 0; i--) {
            ...
            summaryData[`${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`] = 0;
        }
...
function generatePrintView(year, month, period, isDetailed) {
    ...
    let totalDist = 0, totalCount = 0, sumIncomeTrans = 0, sumIncomeOther = 0, sumExpTrans = 0, sumExpGen = 0, sumFuelCost = 0, sumFuelSub = 0, sumFuelLiters = 0;
    const workDays = new Set();

    MEM_RECORDS.forEach(r => {
        const statDate = getStatisticalDate(r.date, r.time);
        if (!statDate.startsWith(`${year}-${month}`)) return;
        const day = parseInt(statDate.split('-')[2]);
        if (day < sDay || day > eDay) return;
        if (r.type === '운행종료' || r.type === '운행취소') return;
        workDays.add(statDate);
        r._statDate = statDate; 
        const cost = safeInt(r.cost); const income = safeInt(r.income);

        if (['화물운송', '대기', '공차이동'].includes(r.type)) {
            transportList.push(r);
            if (r.type === '화물운송') { totalDist += safeFloat(r.distance); totalCount++; }
            sumIncomeTrans += income; sumExpTrans += cost; 
        } else if (r.type === '주유소') {
            fuelList.push(r); sumFuelCost += cost; sumFuelLiters += safeFloat(r.liters); sumFuelSub += safeInt(r.subsidy); 
        } else if (['지출', '소모품', '정비'].includes(r.type)) {
            expenseList.push(r); sumExpGen += cost;
        } else if (r.type === '수입') {
            incomeList.push(r); sumIncomeOther += income;
        }
    });
    ...
    const totalIncome = sumIncomeTrans + sumIncomeOther;
    const totalExpense = sumExpTrans + sumExpGen; 
    const netFuelCost = sumFuelCost - sumFuelSub; 
    ...
    const buildTable = (title, headers, rows) => {
        ...
        if (rows.length === 0) tHtml += `<tr><td colspan="${headers.length}" class="no-record">내역 없음</td></tr>`;
            ...
            let lastDate = '';
            rows.forEach(r => {
                const isNewDay = r._statDate !== lastDate; lastDate = r._statDate;
                tHtml += `<tr class="${isNewDay ? 'new-day' : ''}">${r.cells.map(c=>`<td class="${c.cls||''}">${c.val}</td>`).join('')}</tr>`;
            });
        }
        tHtml += `</tbody></table>`;
        return tHtml;
    ...
    reportContent += buildTable('2. 주유 및 정비 내역',
        ...
        fuelList.map(r => {
            const c=safeInt(r.cost), s=safeInt(r.subsidy);
            return { _statDate: r._statDate, cells: [{val: r.date.substring(5), cls:'txt-center'}, {val: r.liters?r.liters.toFixed(2)+' L':'-', cls:'txt-center'}, {val: r.unitPrice?formatMoney(r.unitPrice)+' 원':'-', cls:'txt-center'}, {val: formatMoney(c)+' 원', cls:'txt-right'}, {val: '-'+formatMoney(s)+' 원', cls:'txt-right txt-red'}, {val: formatMoney(c-s)+' 원', cls:'txt-right val-bold'}]};
        })
    );
    reportContent += buildTable('3. 지출 내역',
        [ {text:'날짜', style:'width:15%'}, {text:'내용 (적요)'}, {text:'지출금액', style:'width:20%'} ],
        expenseList.map(r => ({ _statDate: r._statDate, cells: [{val: r.date.substring(5), cls:'txt-center'}, {val: r.expenseItem||r.supplyItem||r.type}, {val: formatMoney(safeInt(r.cost))+' 원', cls:'txt-right'}]}))
    );
    ...
    modal.id = 'print-modal'; modal.style.position = 'fixed'; modal.style.top = '0'; modal.style.left = '0'; modal.style.width = '100%'; modal.style.height = '100%'; modal.style.zIndex = '9999'; modal.style.overflow = 'auto'; modal.style.background = 'white';
...
function displaySubsidyRecords(showAll = false) {
    const list = document.getElementById('subsidy-records-list');
    if(!list) return; 
    list.innerHTML = '';
    ...
    if (allRecords.length === 0) {
        list.innerHTML = '<p style="text-align:center; padding:20px; color:#999;">저장된 주유 기록이 없습니다.</p>';
        return;
    }
    ...
    let displayList = [];
    if (showAll) {
        displayList = allRecords;
    } else {
        displayList = allRecords.filter(r => r.date.startsWith(currentYM));
    }
    ...
    displayList.forEach(r => {
        ...
        div.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center;">
                <span style="font-weight:bold;">${r.date} <span style="font-size:0.8em; color:#666; font-weight:normal;">(${r.brand || '기타'})</span></span>
                <span style="font-weight:bold; font-size:1.1em;">${formatToManwon(safeInt(r.cost))} 만원</span>
            </div>
            <div style="display:flex; justify-content:space-between; font-size:0.9em; color:#555; margin-top:4px;">
    ...
    if (!showAll && allRecords.length > displayList.length) {
        const moreBtn = document.createElement('button');
        moreBtn.textContent = "▼ 이전 내역 더보기";
        moreBtn.style.width = "100%";
        moreBtn.style.padding = "10px";
        moreBtn.style.marginTop = "10px";
        moreBtn.style.background = "#f8f9fa";
        moreBtn.style.border = "1px solid #ddd";
        moreBtn.style.cursor = "pointer";
        moreBtn.onclick = () => displaySubsidyRecords(true);
...
window.updateAllDisplays = function() {
    ...
    if(todayPicker) displayTodayRecords(todayPicker.value);
    displayDailyRecords();
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // '문자 자동 읽기' 버튼 강제 숨김 처리
    const smsReadBtn = document.getElementById('btn-read-sms');
    if(smsReadBtn) smsReadBtn.style.display = 'none';

    // 모바일 토글 버튼(legend)에 대한 클릭 이벤트 리스너 추가
    document.querySelectorAll('.mobile-toggle-legend').forEach(legend => {
    ...
    if(document.getElementById('today-date-picker')) document.getElementById('today-date-picker').value = today;
    if(document.getElementById('time')) document.getElementById('time').value = getCurrentTimeString();
    ...
    const monthOptions = [];
    for (let i = 1; i <= 12; i++) {
        monthOptions.push(`<option value="${String(i).padStart(2,'0')}">${i}월</option>`);
    }
    
    ['daily','weekly','monthly','print'].forEach(p => {
        const y = document.getElementById(`${p}-year-select`);
        const m = document.getElementById(`${p}-month-select`);
...
window.updateIncomeFromPrint = function(inputEl, id) {
    ...
    document.getElementById('print-second-half-btn')?.addEventListener('click', () => {
        generatePrintView(document.getElementById('print-year-select').value, document.getElementById('print-month-select').value, 'second', false);
    });

    document.getElementById('print-full-month-btn')?.addEventListener('click', () => {
        generatePrintView(document.getElementById('print-year-select').value, document.getElementById('print-month-select').value, 'full', false);
    });

    document.getElementById('print-first-half-detail-btn')?.addEventListener('click', () => {
        generatePrintView(document.getElementById('print-year-select').value, document.getElementById('print-month-select').value, 'first', true);
    });

    document.getElementById('print-second-half-detail-btn')?.addEventListener('click', () => {
        generatePrintView(document.getElementById('print-year-select').value, document.getElementById('print-month-select').value, 'second', true);
    });

    document.getElementById('print-full-month-detail-btn')?.addEventListener('click', () => {
        generatePrintView(document.getElementById('print-year-select').value, document.getElementById('print-month-select').value, 'full', true);
    });

// ==========================================
    // [1] 자동/수동 백업 실행 함수 (이 자리에 붙여넣기)
    // ==========================================
    function performExport(isAuto = false) {
        ...
        if (window.CargoNoteApp && window.CargoNoteApp.saveBackupFile) { 
            window.CargoNoteApp.saveBackupFile(jsonString);
            if(isAuto) showToast("📅 금일 최초 실행: 데이터 자동 백업 중...");
            else showToast("백업 파일 저장 시도 중...");
            return true;
        } 
        // 2. 브라우저/일반 다운로드
        else { 
            if(isAuto) {
                try {
                    ...
                    a.href = url;
                    a.download = `CargoNote_AutoBackup_${timestamp}.json`;
                    document.body.appendChild(a);
                    a.click();
                    document.body.removeChild(a);
                    showToast("📅 자동 백업 파일이 다운로드되었습니다.");
                    return true;
    ...
    document.getElementById('import-file-input')?.addEventListener('change', (e) => { 
        ...
        r.onload = (evt) => { 
            try {
                ...
                if(d.salaries) localStorage.setItem('saved_salaries', JSON.stringify(d.salaries));
    ...
    document.getElementById('btn-save-ocr')?.addEventListener('click', () => {
        const cost = safeInt(document.getElementById('ocr-cost').value);
        if(cost <= 0) { alert('금액 확인 필요'); return; }
        
        const container = document.getElementById('ocr-result-container');
        const subsidy = container.dataset.subsidy ? parseInt(container.dataset.subsidy) : 0;
        const brand = container.dataset.brand || '기타';
    ...
    document.getElementById('ocr-input')?.addEventListener('change', (e) => { 
        if(e.target.files[0]) performOCR(e.target.files[0]); 
    });

    document.getElementById('btn-register-trip')?.addEventListener('click', () => {
        const d = getFormDataWithoutTime();
        if((d.type==='화물운송'||d.type==='대기') && (!d.from || !d.to)) return alert('상하차지 입력 필요');
        addRecord({ id: Date.now(), date: document.getElementById('date').value, time: document.getElementById('time').value, ...d });
        showToast('등록되었습니다.'); resetForm(); updateAllDisplays();
    });
    
    document.getElementById('btn-start-trip')?.addEventListener('click', () => {
        const formData = getFormDataWithoutTime();
        if ((formData.type === '화물운송' || formData.type === '대기') && (!formData.from || !formData.to)) { alert('상차지와 하차지를 모두 입력해주세요.'); return; }
        addRecord({ id: Date.now(), date: getTodayString(), time: getCurrentTimeString(), ...formData });
        showToast('운행 시작됨'); resetForm(); document.getElementById('today-date-picker').value = getStatisticalDate(getTodayString(), getCurrentTimeString()); updateAllDisplays();
    });
    ...
    document.getElementById('btn-end-trip')?.addEventListener('click', () => {
        ...
        showToast('운행 종료됨'); resetForm(); updateAllDisplays();
        if (window.CargoNoteApp && window.CargoNoteApp.startGPSLogging) window.CargoNoteApp.startGPSLogging();
    });
    ...
    document.getElementById('btn-trip-cancel')?.addEventListener('click', () => {
        ...
        showToast('취소 처리됨'); resetForm(); updateAllDisplays();
    });
    ...
    document.getElementById('btn-save-general')?.addEventListener('click', () => {
        ...
        showToast('저장되었습니다.'); populateExpenseDatalist(); resetForm(); updateAllDisplays();
        if(formData.type === '주유소') displaySubsidyRecords(false);
    });
...
document.getElementById('btn-update-record')?.addEventListener('click', () => {
        ...
        if (index > -1) {
            ...
            // 이제 newDate와 newTime을 사용할 수 있습니다.
            MEM_RECORDS[index] = { ...original, ...formData, date: newDate, time: newTime };
            saveData(); showToast('수정 완료'); resetForm(); updateAllDisplays();
        }
    });
    ...
    document.getElementById('btn-edit-start-trip')?.addEventListener('click', () => {
        const id = parseInt(document.getElementById('edit-id').value);
        const index = MEM_RECORDS.findIndex(r => r.id === id);
        if (index > -1) { MEM_RECORDS[index].date = getTodayString(); MEM_RECORDS[index].time = getCurrentTimeString(); saveData(); resetForm(); updateAllDisplays(); }
    });
    
    document.getElementById('btn-edit-end-trip')?.addEventListener('click', () => {
        addRecord({ id: Date.now(), date: getTodayString(), time: getCurrentTimeString(), type: '운행종료', distance: 0, cost: 0, income: 0 }); resetForm(); updateAllDisplays();
    });
    ...
    document.getElementById('prev-monthly-btn')?.addEventListener('click', () => {
        const yEl = document.getElementById('monthly-year-select'); if(yEl) { yEl.value = parseInt(yEl.value) - 1; updateAllDisplays(); }
    });
    document.getElementById('next-monthly-btn')?.addEventListener('click', () => {
        const yEl = document.getElementById('monthly-year-select'); if(yEl) { yEl.value = parseInt(yEl.value) + 1; updateAllDisplays(); }
    });
    ...
    document.addEventListener('click', (e) => {
        ...
        if(e.target.classList.contains('location-clickable')) {
            ...
            addrTarget = e.target.closest('.address-clickable');
        }

        if (addrTarget) {
            let addr = addrTarget.dataset.address;
            const centerName = addrTarget.dataset.center || addrTarget.textContent.trim();
            
            if(!addr && centerName && MEM_LOCATIONS[centerName]) {
                addr = MEM_LOCATIONS[centerName].address;
            }

            if (addr) copyTextToClipboard(addr, `[${centerName}] 주소 복사됨`);
            else showToast(`'${centerName}' 저장된 주소 없음`);
        }
    });

    const toggleSmsBtn = document.getElementById('toggle-sms-section-btn');
    if (toggleSmsBtn) {
        toggleSmsBtn.addEventListener('click', () => window.toggleSection('sms-parser-section', toggleSmsBtn));
    }
    
    // 기록일시/기록종류 토글 이벤트 연결 (다중 ID 체크)
    const toggleBasicBtn = document.getElementById('toggle-basic-info-btn');
    if(toggleBasicBtn) {
        toggleBasicBtn.addEventListener('click', () => window.toggleSection('basic-info-section', toggleBasicBtn));
    }
    const toggleDateBtn = document.getElementById('toggle-date-btn');
    if(toggleDateBtn) {
        toggleDateBtn.addEventListener('click', () => window.toggleSection('date-fieldset', toggleDateBtn));
    }
    const toggleTypeBtn = document.getElementById('toggle-type-btn');
    if(toggleTypeBtn) {
        toggleTypeBtn.addEventListener('click', () => window.toggleSection('type-fieldset', toggleTypeBtn));
    }

    const toggleLocBtn = document.getElementById('toggle-location-section-btn');
    if (toggleLocBtn) {
        toggleLocBtn.addEventListener('click', () => window.toggleSection('transport-details', toggleLocBtn));
    }
    const toggleCostBtn = document.getElementById('toggle-cost-section-btn');
    if (toggleCostBtn) {
        toggleCostBtn.addEventListener('click', () => window.toggleSection('cost-info-fieldset', toggleCostBtn));
    }
    const toggleTodayBtn = document.getElementById('toggle-today-list-btn');
    if (toggleTodayBtn) {
        toggleTodayBtn.addEventListener('click', () => window.toggleSection('today-records-table', toggleTodayBtn));
    }

    // 지역 관리 검색 이벤트 연결
    const searchInput = document.getElementById('center-search-input');
    if (searchInput) {
        searchInput.addEventListener('input', (e) => displayCenterList(e.target.value));
    }
    
    // 새 지역 추가 버튼 연결
    const addCenterBtn = document.getElementById('btn-add-center') || document.querySelector('button[onclick*="addNewCenterFromInput"]');
    if (addCenterBtn) {
        addCenterBtn.addEventListener('click', window.addNewCenterFromInput);
    } else {
        const settingsPage = document.getElementById('settings-page');
        if(settingsPage) {
            const btns = settingsPage.querySelectorAll('button');
            for(let i=0; i<btns.length; i++) {
                if(btns[i].textContent.includes('추가') || btns[i].textContent.includes('등록')) {
                    btns[i].addEventListener('click', window.addNewCenterFromInput);
                    break;
                }
            }
        }
    ...
    const dataBody = document.getElementById('data-management-body');
    if(dataBody && !document.getElementById('sms-target-number')) {
        let div = document.createElement('div');
        div.className = "input-group";
        div.style.marginBottom = "10px";
        div.innerHTML = `<input type="tel" id="sms-target-number" placeholder="자동 입력할 발신번호 (예: 1544-0000)" value="${localStorage.getItem('target_sms_number') || ''}"><button type="button" id="save-sms-number-btn" style="background-color:#17a2b8;">번호 저장</button>`;
        dataBody.insertBefore(div, dataBody.firstChild);
        
        document.getElementById('save-sms-number-btn').addEventListener('click', () => { 
            localStorage.setItem('target_sms_number', document.getElementById('sms-target-number').value.trim()); 
            showToast("번호 저장됨"); 
            displaySavedSmsNumber();
        });
    }
