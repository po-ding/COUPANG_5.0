...
// ==========================================
// SMS, SALARY, FIXED EXPENSE MANAGER
// ==========================================
...
function displaySavedSmsNumber() {
    ...
    if (!displayDiv) {
        ...
        if(parent) { 
            ...
            newDiv.style.marginTop = '5px'; newDiv.style.marginBottom = '10px'; newDiv.style.fontSize = '0.9em'; newDiv.style.color = '#007bff'; 
            if(parent.children.length > 0) parent.insertBefore(newDiv, parent.children[1]); 
            else parent.appendChild(newDiv); 
            renderSmsNumberContent(newDiv, savedNum); 
        }
    } else { renderSmsNumberContent(displayDiv, savedNum); }
}

function renderSmsNumberContent(element, num) {
    if (num) element.innerHTML = `<div style="display:flex; justify-content:space-between; align-items:center; background:#f1f8ff; padding:8px; border-radius:4px;"><span>저장된 번호: <strong>${num}</strong></span><button type="button" onclick="window.deleteSavedSmsNumber()" style="background:#dc3545; color:white; border:none; border-radius:3px; padding:4px 10px; font-size:0.8em; cursor:pointer;">삭제</button></div>`;
    else element.innerHTML = "<div style='padding:5px; color:#666;'>저장된 발신번호가 없습니다.</div>";
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function performExport(isAuto = false) {
        ...
        if (window.CargoNoteApp && window.CargoNoteApp.saveBackupFile) { window.CargoNoteApp.saveBackupFile(jsonString); if(isAuto) showToast("📅 금일 최초 실행: 데이터 자동 백업 중..."); else showToast("백업 파일 저장 시도 중..."); return true; } 
        else { 
            if(isAuto) {
                try { const blob = new Blob([jsonString], { type: "application/json" }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); const timestamp = new Date().toISOString().replace(/[:.]/g, '-'); a.href = url; a.download = `CargoNote_AutoBackup_${timestamp}.json`; document.body.appendChild(a); a.click(); document.body.removeChild(a); showToast("📅 자동 백업 파일이 다운로드되었습니다."); return true; } catch(e) { return false; }
            } else { if(confirm("안드로이드 파일 저장이 지원되지 않는 환경입니다.\n데이터를 클립보드에 복사하시겠습니까?")) { copyTextToClipboard(jsonString, "데이터가 복사되었습니다."); } return false; }
        }
    }
    ...
    document.getElementById('btn-start-trip')?.addEventListener('click', () => {
        ...
        addRecord({ id: Date.now(), date: getTodayString(), time: getCurrentTimeString(), ...formData }); showToast('운행 시작됨'); resetForm(); document.getElementById('today-date-picker').value = getStatisticalDate(getTodayString(), getCurrentTimeString()); updateAllDisplays();
    });
    document.getElementById('btn-end-trip')?.addEventListener('click', () => {
        addRecord({ id: Date.now(), date: getTodayString(), time: getCurrentTimeString(), type: '운행종료', distance: 0, cost: 0, income: 0 }); showToast('운행 종료됨'); resetForm(); updateAllDisplays(); if (window.CargoNoteApp && window.CargoNoteApp.startGPSLogging) window.CargoNoteApp.startGPSLogging();
    });
    document.getElementById('btn-trip-cancel')?.addEventListener('click', () => {
        const formData = getFormDataWithoutTime(); addRecord({ id: Date.now(), date: getTodayString(), time: getCurrentTimeString(), ...formData, type: '운행취소' }); showToast('취소 처리됨'); resetForm(); updateAllDisplays();
    });
    document.getElementById('btn-save-general')?.addEventListener('click', () => {
        const formData = getFormDataWithoutTime(); if (formData.expenseItem) updateExpenseItemData(formData.expenseItem);
        addRecord({ id: Date.now(), date: document.getElementById('date').value, time: document.getElementById('time').value, ...formData }); showToast('저장되었습니다.'); populateExpenseDatalist(); resetForm(); updateAllDisplays(); if(formData.type === '주유소') displaySubsidyRecords(false);
    });
    document.getElementById('btn-update-record')?.addEventListener('click', () => {
        const id = parseInt(document.getElementById('edit-id').value); const index = MEM_RECORDS.findIndex(r => r.id === id);
        if (index > -1) {
            const original = MEM_RECORDS[index]; const formData = getFormDataWithoutTime(); const newDate = document.getElementById('date').value; const newTime = document.getElementById('time').value;
            if (formData.type === '화물운송' && formData.from && formData.to) { const key = `${formData.from}-${formData.to}`; if(formData.distance > 0) MEM_DISTANCES[key] = formData.distance; if(formData.income > 0) MEM_FARES[key] = formData.income; }
            MEM_RECORDS[index] = { ...original, ...formData, date: newDate, time: newTime }; saveData(); showToast('수정 완료'); resetForm(); updateAllDisplays();
        }
    });
    ...
    window.onDistanceReceived = function(distanceKm) {
        if (!distanceKm || distanceKm <= 0) return; const floatDist = parseFloat(distanceKm).toFixed(1);
        if (confirm(`측정된 거리: ${floatDist}km\n[확인] = 운행거리에 입력 (화물운송)\n[취소] = 공차이동으로 저장`)) { const distInput = document.getElementById('manual-distance'); if (distInput) { distInput.value = floatDist; distInput.dispatchEvent(new Event('input')); } } 
        else { if(confirm("공차 이동 기록으로 저장하시겠습니까?")) { addRecord({ id: Date.now(), date: getTodayString(), time: getCurrentTimeString(), type: '공차이동', distance: parseFloat(floatDist), cost: 0, income: 0, from: 'GPS 측정', to: '이동' }); showToast("공차이동 저장됨"); updateAllDisplays(); } }
    };
