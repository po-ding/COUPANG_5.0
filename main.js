// ==========================================
// PART 1: DATA LAYER & GLOBAL SETTINGS
// ==========================================
...
// ==========================================
// PART 4: STATISTICS, DAILY RECORDS & PRINT
// ==========================================
...
// ==========================================
// PART 5-1: BASIC UTILITIES & DATE FUNCTIONS
// ==========================================
...
// ==========================================
// PART 5-2: UI TOGGLE & LIST DISPLAY
// ==========================================
...
// ==========================================
// PART 5-3: FORM DATA HANDLING & EDIT MODE
// ==========================================
...
function registerParsedTripWithInfo(btn, lineIdx) {
    ...
    const smsSection = document.getElementById('sms-parser-section'); 
    if(smsSection) { smsSection.classList.remove('hidden'); smsSection.style.display = 'block'; }
}
...
function displayMonthlyRecords() {
    ...
    MEM_FIXED_EXPENSES.forEach(fe => { if(fe.month === 0) totalFixedExpense += (fe.cost * 12); else totalFixedExpense += fe.cost; });
    let totalYearSalary = 0; Object.keys(MEM_SALARIES).forEach(k => { if(k.startsWith(year)) totalYearSalary += (MEM_SALARIES[k] || 0); });
    let summaryHtml = createSummaryHTML(`${year}년`, yearRecords);
    if(totalYearSalary > 0) summaryHtml += `<div style="margin-top:5px; text-align:right; color:#007bff; font-weight:bold; font-size:0.9em;">➕ 연간 급여 합계: ${formatToManwon(totalYearSalary)} 만원</div>`;
    if(totalFixedExpense > 0) summaryHtml += `<div style="text-align:right; color:#dc3545; font-weight:bold; font-size:0.9em;">➖ 연간 고정 지출 (추정): 약 ${formatToManwon(totalFixedExpense)} 만원</div>`;
    ...
    Object.keys(months).sort().reverse().forEach(m => {
        ...
        let fixedMonthly = 0;
        const currentMonthNum = parseInt(m.substring(5));
        MEM_FIXED_EXPENSES.forEach(fe => { if (fe.month === 0 || fe.month === currentMonthNum) { fixedMonthly += fe.cost; } });
        exp += fixedMonthly;
...
function generatePrintView(year, month, period, isDetailed) {
    ...
    // HTML 리포트 (기호 제거)
    let reportContent = `<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><title>운송 기록 [${year}-${month}]</title><style>body { font-family: 'Malgun Gothic', sans-serif; padding: 20px; color: #333; background: #fff; }h2 { text-align: left; border-bottom: 2px solid #333; padding-bottom: 10px; margin-bottom: 20px; font-size: 1.5em; }.summary-box { background: #f9f9f9; padding: 15px; border: 1px solid #ddd; border-radius: 5px; margin-bottom: 30px; font-size: 0.95em; line-height: 1.8; }.summary-row { margin-bottom: 5px; }.plus { color: #007bff; } .minus { color: #dc3545; } .equal { color: #28a745; }.val-bold { font-weight: bold; }.section-title { font-size: 1.1em; font-weight: bold; margin: 20px 0 10px 0; border-left: 5px solid #333; padding-left: 10px; }table { width: 100%; border-collapse: collapse; font-size: 12px; margin-bottom: 20px; }th { background: #eee; border: 1px solid #ccc; padding: 8px; font-weight: bold; text-align: center; }td { border-left: 1px solid #ccc; border-right: 1px solid #ccc; padding: 6px 8px; vertical-align: middle; }tr.new-day td { border-top: 2px solid #444 !important; } tr:not(.new-day) td { border-top: 1px solid #eee; }.txt-right { text-align: right; }.txt-center { text-align: center; }.txt-red { color: #dc3545; }.txt-blue { color: #007bff; }.no-record { text-align: center; padding: 20px; color: #999; border-bottom: 1px solid #ccc; }input.money-input { width: 60px; text-align: right; border: none; border-bottom: 1px solid #ccc; font-family: inherit; font-size: inherit; background: transparent; }@media print { input { border: none !important; border-bottom: none !important; } }</style></head><body><h2>${year}년 ${month}월 ${periodStr} 운송 기록</h2><div class="summary-box"><div class="summary-row"><strong>[요약]</strong> 근무일: ${workDays.size}일 | 운행건수: ${totalCount}건 | 운행거리: ${totalDist.toFixed(1)}km</div><div class="summary-row" style="margin-top:10px; border-top:1px dashed #ccc; padding-top:10px;">총 수입: <span class="val-bold plus">${formatMoney(totalIncome)} 원</span> <span style="color:#666; font-size:0.9em;">(운송: ${formatMoney(sumIncomeTrans)} + 기타/급여: ${formatMoney(sumIncomeOther)})</span></div><div class="summary-row">총 지출: <span class="val-bold minus">${formatMoney(totalExpense)} 원</span> <span style="color:#666; font-size:0.9em;">(운송지출: ${formatMoney(sumExpTrans)} + 일반지출: ${formatMoney(sumExpGen)})</span></div><div class="summary-row">실 주유비: <span class="val-bold minus">${formatMoney(netFuelCost)} 원</span> <span style="color:#666; font-size:0.9em;">(주유금액: ${formatMoney(sumFuelCost)} - 보조금: ${formatMoney(sumFuelSub)})</span></div><div class="summary-row" style="margin-top:10px; border-top:1px solid #ccc; padding-top:10px; font-size:1.1em;">최종 순수익: <span class="val-bold equal" style="font-size:1.2em;">${formatMoney(finalProfit)} 원</span></div></div>`;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const cancelBtn = document.getElementById('btn-trip-cancel');
    if(cancelBtn) cancelBtn.textContent = "운행삭제";
    ...
    function performExport(isAuto = false) { 
        const data = { records: MEM_RECORDS, centers: MEM_CENTERS, locations: MEM_LOCATIONS, fares: MEM_FARES, distances: MEM_DISTANCES, costs: MEM_COSTS, subsidy: localStorage.getItem('fuel_subsidy_limit'), correction: localStorage.getItem('mileage_correction'), expenseItems: MEM_EXPENSE_ITEMS, salaries: MEM_SALARIES, fixedExpenses: MEM_FIXED_EXPENSES }; 
        ...
        if (window.CargoNoteApp && window.CargoNoteApp.saveBackupFile) { 
            ...
            if(isAuto) { 
                try { 
                    ...
                    a.href = url; a.download = `CargoNote_AutoBackup_${timestamp}.json`; 
                    document.body.appendChild(a); a.click(); document.body.removeChild(a); 
                    showToast("📅 자동 백업 파일이 다운로드되었습니다."); return true; 
                } catch(e) { return false; } 
            } else { 
                if(confirm("안드로이드 파일 저장이 지원되지 않는 환경입니다.\n데이터를 클립보드에 복사하시겠습니까?")) { copyTextToClipboard(jsonString, "데이터가 복사되었습니다."); } 
                return false; 
            } 
        } 
    }
    ...
    function checkAndAutoBackup() { 
        ...
        if (lastBackupDate !== today) { 
            setTimeout(() => { 
                ...
                if (success) { localStorage.setItem('last_auto_backup_date', today); } 
            }, 1500); 
        } 
    }
    ...
    document.getElementById('btn-save-ocr')?.addEventListener('click', () => { 
        ...
        addRecord({ id: Date.now(), date: document.getElementById('ocr-date').value, time: document.getElementById('ocr-time').value, type: '주유소', cost: cost, liters: safeFloat(document.getElementById('ocr-liters').value), unitPrice: safeInt(document.getElementById('ocr-price').value), brand: brand, subsidy: subsidy }); 
        showToast('저장됨'); 
        document.getElementById('ocr-result-container').classList.add('hidden'); 
    ...
    document.getElementById('btn-trip-cancel')?.addEventListener('click', () => {
        const editIdVal = document.getElementById('edit-id').value;
        if(editIdVal) {
            if(confirm('정말 이 운행 기록을 삭제하시겠습니까?')) {
                removeRecord(parseInt(editIdVal));
                showToast('삭제되었습니다.');
                resetForm();
                updateAllDisplays();
            }
    ...
    const smsInput = document.getElementById('sms-input'); 
    if(smsInput) { 
        smsInput.addEventListener('input', function() { this.style.height = '1px'; this.style.height = (this.scrollHeight)+'px'; }); 
    }
    ...
    document.addEventListener('click', (e) => { 
        ...
        let addrTarget = null; 
        if(e.target.classList.contains('location-clickable')) { addrTarget = e.target; } 
        else { addrTarget = e.target.closest('.address-clickable'); } 
        if (addrTarget) { 
            let addr = addrTarget.dataset.address; 
            const centerName = addrTarget.dataset.center || addrTarget.textContent.trim(); 
            if(!addr && centerName && MEM_LOCATIONS[centerName]) { addr = MEM_LOCATIONS[centerName].address; } 
    ...
    if (searchInput) { searchInput.addEventListener('input', (e) => displayCenterList(e.target.value)); }
    
    const addCenterBtn = document.getElementById('btn-add-center') || document.querySelector('button[onclick*="addNewCenterFromInput"]'); 
    if (addCenterBtn) { addCenterBtn.addEventListener('click', window.addNewCenterFromInput); } 
    ...
    if(dataBody && !document.getElementById('sms-target-number')) { 
        let div = document.createElement('div'); div.className = "input-group"; div.style.marginBottom = "10px"; 
        ...
        document.getElementById('save-sms-number-btn').addEventListener('click', () => { localStorage.setItem('target_sms_number', document.getElementById('sms-target-number').value.trim()); showToast("번호 저장됨"); displaySavedSmsNumber(); }); 
    }
    ...
    window.onDistanceReceived = function(distanceKm) { 
        ...
        if (confirm(`측정된 거리: ${floatDist}km\n[확인] = 운행거리에 입력 (화물운송)\n[취소] = 공차이동으로 저장`)) { 
            ...
            if (distInput) { distInput.value = floatDist; distInput.dispatchEvent(new Event('input')); } 
        } else { 
            if(confirm("공차 이동 기록으로 저장하시겠습니까?")) { 
                addRecord({ id: Date.now(), date: getTodayString(), time: getCurrentTimeString(), type: '공차이동', distance: parseFloat(floatDist), cost: 0, income: 0, from: 'GPS 측정', to: '이동' }); 
                showToast("공차이동 저장됨"); updateAllDisplays(); 
            } 
        } 
    };
